...
export const discordAuthRoutes: FastifyPluginAsync = async (app) => {
  app.post("/api/discord/resume", async (request, reply) => {
    ...
    const parsed = z.object({ instanceId: z.string().min(1).max(256), guildId: z.string().min(1).optional(), channelId: z.string().min(1).optional(), userId: z.string().min(1) }).safeParse(request.body);
    const accessToken = getBearerToken(request.headers.authorization);
    if (!parsed.success || !accessToken || accessToken.length > 4096) return reply.code(400).send(apiError("invalid_request", "Invalid Discord session recovery request."));
    try {
      const user = await verifyDiscordAuthorization(app.envConfig, accessToken);
      if (user.id !== parsed.data.userId) throw new DiscordActivityError("discord_actor_forbidden", 403);
      const discordContext = await verifyDiscordActivityContext(app.envConfig, parsed.data, user.id);
      const { appToken, session } = await createAppSession({ env: app.envConfig, user, discordContext });
      return reply.send({ appToken, discordAccessToken: accessToken, user, expiresAt: session.expiresAt.toISOString() });
    } catch (error) {
      // Never log the supplied bearer or upstream payload during recovery.
      if (error instanceof DiscordActivityError || error instanceof DiscordOAuthError) {
        const status = [401, 403].includes(error.statusCode) ? error.statusCode : 503;
        return reply.code(status).send(apiError("discord_resume_failed", status === 503 ? "Discord is temporarily unavailable. Try connecting again." : "Discord could not restore access. Close this Activity and open it again."));
      }
      return reply.code(503).send(apiError("discord_resume_failed", "Discord could not restore the connection. Try connecting again."));
    }
  });
  ...
  app.post("/api/discord/exchange", async (request, reply) => {
    ...
    try {
      ...
      if (error instanceof DiscordOAuthError) {
        return reply.code(error.statusCode === 503 ? 503 : 502).send(apiError(error.code, "Discord could not complete sign-in. Try connecting again."));
      }
